...
func Get(path string) (Interface, error) {
	...
	ctxt, err := func(fname string) (map[string]interface{}, error) {
		...
		return metadata, nil
	}(filepath.Join(absPath, templatectl.ContextFileName))

	metadataExists, err := osutil.FileExists(filepath.Join(absPath, templatectl.TemplateMetadataName))
	if err != nil {
		return nil, err
	}
	...
	md, err := func() (Metadata, error) {
		...
		b, err := ioutil.ReadFile(filepath.Join(absPath, templatectl.TemplateMetadataName))
		if err != nil {
			return Metadata{}, err
		}
	...
	return &dirTemplate{
		Context:  ctxt,
		FuncMap:  FuncMap,
		Path:     filepath.Join(absPath, templatectl.TemplateDirName),
		Metadata: md,
	}, err
}
