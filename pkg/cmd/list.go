...
import (
	...
	"github.com/4thel00z/templatectl/pkg/template"
	"github.com/4thel00z/templatectl/pkg/util/exit"
	"github.com/4thel00z/templatectl/pkg/util/tabular"
...
func ListTemplates() (map[string]bool, error) {
	d, err := os.Open(templatectl.Configuration.TemplateDirPath)
	if err != nil {
		return nil, err
	}
	defer d.Close()
...
var List = &cli.Command{
	...
	Run: func(cmd *cli.Command, args []string) {
		...
		var data [][]string
		for _, name := range names {
			tmplPath, err := templatectl.TemplatePath(name)
			if err != nil {
