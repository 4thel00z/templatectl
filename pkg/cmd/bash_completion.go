...
func configureBashCompletion() error {
	bashCompletionFilePath := filepath.Join(templatectl.Configuration.ConfigDirPath, "completion.bash")
	...
	bashrcText := `
# Enables command-line completion for templatectl
source %s
`

	bashrcText = fmt.Sprintf(bashrcText, filepath.Join("$HOME", templatectl.ConfigDirPath, "completion.bash"))
